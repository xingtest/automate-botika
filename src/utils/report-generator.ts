...
export async function generateReportFromLatestTest(openInBrowser: boolean = false): Promise<string | null> {
  ...
  if (!jsonFile) {
    console.log('⚠️ No test files found. Generating ERROR REPORT fallback.');
    const dateNow = new Date();
    baseName = `Failed_Run_${dateNow.getTime()}`;

    summary = {
      id_test: 'FAILURE',
      tester_name: 'GitHub Actions',
      date_test: dateNow.toISOString().split('T')[0],
      start_time_test: '-',
      end_time_test: '-',
      ai_evaluation: 'CRITICAL: Test execution failed to produce data',
    ...
    testData.push({
      no: 1,
      title: 'System Error',
      question: 'Check workflow logs',
      response_kb: '-',
      response_llm: 'Test executed but valid data was not produced.',
      explanation: 'This usually means the browser crashed or the test script exited prematurely.',
      image_capture: '',
      skor: 0,
      status: 'FAILED',
      duration: '0s'
    });
    chartData = [{ name: 'System Error', value: 1 }];
    ...
    try {
      testData = JSON.parse(fs.readFileSync(jsonPath, 'utf-8'));
      ...
      if (fs.existsSync(chartPath)) {
        ...
        // Create simple chart data if missing
        chartData = testData.map(item => ({ [item.title || 'Unknown']: 10 }));
      }
